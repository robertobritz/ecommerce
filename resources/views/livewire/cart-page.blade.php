            ...
            <table class="w-full">
              <thead>
                <tr>
                  ...
                  <th class="font-semibold text-left">Excluir</th>
                </tr>
              </thead>
              <tbody>
                ...
                @forelse ($cart_items as $item )
                    <tr wire:key='{{ $item['product_id'] }}'>
                        ...
                        <td><button wire:click='removeItem({{ $item['product_id'] }})' class="px-3 py-1 border-2 rounded-lg bg-slate-300 border-slate-400 hover:bg-red-500 hover:text-white hover:border-red-700"><span wire:loading.remove wire:target='removeItem({{ $item['product_id'] }})'>Excluir</span><span wire:loading wire:target='removeItem({{ $item['product_id'] }})'>Excluindo</span></button></td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="py-4 text-4xl font-semibold text-center text-slate-500"> Sem nenhum produto no carrinho! </td>
                    </tr>
                @endforelse
              </tbody>
            </table>

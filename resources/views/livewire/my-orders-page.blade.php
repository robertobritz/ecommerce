                ...
                @foreach ($orders as $order )
                    @php
                        ...
                        if($order->payment_status == 'pending'){
                            $payment_status = '<span class="px-3 py-1 text-white bg-blue-500 shadow roundee">Pending</span>';
                        }
                        if($order->payment_status == 'paid'){
                            $payment_status = '<span class="px-3 py-1 text-white bg-green-500 shadow roundee">Paid</span>';
                        }
                        if($order->payment_status == 'failed'){
                            $payment_status = '<span class="px-3 py-1 text-white bg-red-500 shadow roundee">Failed</span>';
                        }
                    @endphp
                    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key='{{ $order->id }}'>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-800 whitespace-nowrap dark:text-gray-200">{{ Number::currency($order->grand_total, 'Brl') }}</td>
